...
function piscar(numeroBotao) {
    setTimeout(() => {
        analisarSelecionarId(numeroBotao)
    }, 1000);

    setTimeout(() => {
        padronizarId(numeroBotao)
    }, 1600)
}
...
function iniciar() {
    ...
    setTimeout(() => {
        textoMeio.innerText = 'Prepare-se e boa sorte'
    }, 1700)
    //txt pedindo atenção aos botões ascendendo
    setTimeout(() => {
        textoMeio.innerText = 'Preste atenção na sequência'
    }, 3000)
    //jogo começa de fato
...
function clicarPontuar(e) {
    ...
    if (e.target === botaoVerde) {
        piscarComClick(1)
    }
    if (e.target === botaoVermelho) {
        piscarComClick(2)
    }
    if (e.target === botaoAmarelo) {
        piscarComClick(3)
    }
    if (e.target === botaoAzul) {
        piscarComClick(4)
    }
    ...
    if (clicados.length === valores.length && valores.every((item, index) => item === clicados[index])) {
        ...
        placar += 1
        if (maiorPontuacao < placar) {
            maiorPontuacao = placar
        }
